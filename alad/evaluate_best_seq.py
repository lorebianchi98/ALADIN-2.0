"""
Effettuate the test on the validation set changing progressively the max sequence length parameters.
Results (the rsum) are stored in a text file. In this way the best combination can be used to improve test set results.
"""
...
def main():
    ...
    parser.add_argument("--max_img_seq_length", default=0, type=int,
                        help="The maximum total input image sequence length. Not required in the valutation")
    ...
    parser.add_argument('--step', type=int, default=5, help='step the determinate the variations of the max sequence length')
    parser.add_argument('--start', type=int, default=5, help='max sequence length initial value')
    parser.add_argument('--end', type=int, default=5, help='max sequence length end value')
